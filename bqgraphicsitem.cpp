...
void BGraphicsItem::updateColor(QColor color)
{
    m_pointList.setColor(color);
    m_pen_noSelected.setColor(color);
    setPen(m_pen_noSelected);
    update();
}

void BGraphicsItem::updateLineWidth(int lineWidth)
{
    if(lineWidth<1)lineWidth=1;
    m_pen_noSelected.setWidth(lineWidth);
    setPen(m_pen_noSelected);
    update();
}
...
BRectangle::BRectangle(qreal x, qreal y, qreal width, qreal height, ItemType type, QPen pen)
    : BGraphicsItem(QPointF(x,y), QPointF(width/2,height/2), type)
{
    BPointItem *point = new BPointItem(this, m_edge, BPointItem::Edge);
    point->setParentItem(this);
    m_pointList.append(point);
    m_pointList.append(new BPointItem(this, m_center, BPointItem::Center));
    m_pointList.setColor(pen.color());
    m_pen_noSelected = pen;
}
...
BEllipse::BEllipse(qreal x, qreal y, qreal width, qreal height, ItemType type, QPen pen)
    : BGraphicsItem(QPointF(x,y), QPointF(x+width/2,y+height/2), type)
{
    BPointItem *point = new BPointItem(this, m_edge, BPointItem::Edge);
    point->setParentItem(this);
...
BPolygon::BPolygon(const QList<QPointF> &points, ItemType type, const QPen &pen)
    : BGraphicsItem(QPointF(0, 0), QPointF(0, 0), type) {
    QList<QPointF> validPoints = points;

    while (validPoints.size() < 3) {// 少于3个点则不能构成多边形，此时未避免程序出错，增加随机的点直至满足点数的条件
        qreal x = QRandomGenerator::global()->bounded(100.0);
        qreal y = QRandomGenerator::global()->bounded(100.0);
        validPoints.append(QPointF(x, y));
    }

    m_pen_noSelected = pen;
    // 初始化多边形的顶点
    for (const auto &point : validPoints) {
        BPointItem *bPointItem = new BPointItem(this, point, BPointItem::Edge);
        bPointItem->setParentItem(this);
        m_pointList.append(bPointItem);
    }

    m_center = getCentroid(validPoints);
    getMaxLength();
    BPointItem *centerItem = new BPointItem(this, m_center, BPointItem::Center);
    centerItem->setParentItem(this);
    m_pointList.append(centerItem);
    m_pointList.setColor(pen.color());
    is_create_finished = true;
    this->update();
}

BPolygon::BPolygon(qreal x, qreal y, qreal width, qreal height, ItemType type, const QPen &pen)
    : BPolygon({QPointF(x - width / 2, y - height / 2),
                QPointF(x + width / 2, y - height / 2),
                QPointF(x + width / 2, y + height / 2),
                QPointF(x - width / 2, y + height / 2)},
               type, pen) {
    // 该构造函数的主体现在为空，因为所有初始化工作都在新构造函数中完成
}
//BPolygon::BPolygon(qreal x, qreal y, qreal width, qreal height, ItemType type, const QPen& pen)// BPolygon的四边形构造函数实现
//    : BGraphicsItem(QPointF(x, y), QPointF(width / 2, height / 2), type) {
//    // 设置颜色和完成标志
//    m_pen_noSelected = pen;
//    QPointF topLeft(x - width / 2, y - height / 2);
//    QPointF topRight(x + width / 2, y - height / 2);
//    QPointF bottomRight(x + width / 2, y + height / 2);
//    QPointF bottomLeft(x - width / 2, y + height / 2);
//    // 初始化多边形的顶点
...
//    }
//    // 设置中心点
//    m_center = getCentroid(points);
//    getMaxLength();
//    BPointItem *centerItem = new BPointItem(this, m_center, BPointItem::Center);
//    centerItem->setParentItem(this);
//    m_pointList.append(centerItem);
//    m_pointList.setColor(pen.color());
//    is_create_finished = true; // 标记为创建完成
//    this->update();
//}
void BPolygon::pushPoint(QPointF p, QList<QPointF> list, bool isCenter)
{
    if (!is_create_finished) {
        ...
        if (isCenter) {
            m_pointList.append(new BPointItem(this, m_center, BPointItem::Center));
            m_pointList.setColor(m_pen_noSelected.color());
            is_create_finished = true;
        } else {
    ...
    : BCircle(x, y, radius1, type), m_another_edge(x+radius2*sqrt(2)/2, y+radius2*sqrt(2)/2)
{
    ...
    m_pointList.append(point);
    //m_pointList.setRandColor();
    qDebug()<<"2 m_pen_noSelected.color()"<<m_pen_noSelected.color();
    m_pointList.setColor(m_pen_noSelected.color());
    updateOtherRadius();
}
    ...
    : BRectangle(x, y, width, height, type)
{
    ...
    m_pointList.append(point);
    //m_pointList.setRandColor();
    qDebug()<<"6 m_pen_noSelected.color()"<<m_pen_noSelected.color();
    m_pointList.setColor(m_pen_noSelected.color());
