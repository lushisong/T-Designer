...
void SelectFunctionDialog::ShowSet()
{
    ...
    layout->addWidget(ui->functionTree);
}
        ...
        case 0:
            ...
            else
            {
                if (varType == "Bool") {
                    ...
                    if(testItemList[row].variable.endsWith("u")) CbTestVal2->setList(obsTemplates["AC380_3P_u"]);
                    if(testItemList[row].variable.endsWith("i")) CbTestVal2->setList(obsTemplates["AC380_3P_i"]);
                    if(testItemList[row].checkState == Qt::Checked) testItemList[row].confidence = 1.0;
                    if(testItemList[row].checkState == Qt::Unchecked) testItemList[row].confidence = 0.1;
                } else if (varType == "Real") {
                    if(testItemList[row].variable.endsWith("u")) CbTestVal2->setList(obsTemplates["AC220_1P_u"]);
                    if(testItemList[row].variable.endsWith("i")) CbTestVal2->setList(obsTemplates["AC220_1P_i"]);
                    if(testItemList[row].checkState == Qt::Checked) testItemList[row].confidence = 1.0;
                    if(testItemList[row].checkState == Qt::Unchecked) testItemList[row].confidence = 0.1;
                } else {
...
void SelectFunctionDialog::on_functionTree_itemClicked(QTreeWidgetItem *item, int column) {
    ...
    // 更新求解空间
    QString link = doc.firstChildElement("functiondefine").firstChildElement("link").text();
    ui->textEditLink->setPlainText(link);

    // 更新依赖信息
    QString functionDependency = doc.firstChildElement("functiondefine").firstChildElement("dependency").firstChildElement("function").text();
    QString componentDependency = doc.firstChildElement("functiondefine").firstChildElement("dependency").firstChildElement("component").text();
    ...
    //更新全部相关器件
    QString allComponent = doc.firstChildElement("functiondefine").firstChildElement("dependency").firstChildElement("allComponent").text();
    ...
    QDomNodeList constraints = doc.firstChildElement("functiondefine").elementsByTagName("constraint");
...
QTreeWidget* SelectFunctionDialog::GetTreeWidget()
{
    return ui->functionTree;
}
...
void SelectFunctionDialog::on_btn_UpdateSubFunc_clicked()
{
    ...
    for (int i = 0; i < ui->functionTree->topLevelItemCount(); ++i) {
        ...
        functionAllComponentMap.insert(mFunc.functionName, allComponent);
    ...
    else
    {
        ...
        for(const resultEntity &res : functionResultEntity) {
            ...
            if(componentsInEntityString.contains(",")) {
                // 把","替换成空格，并用括号括起来
                QString componentsWithBrackets = "(" + componentsInEntityString.replace(",", " ") + ")";
                // 作为单个元素加入到componentSet中
                componentSet.insert(componentsWithBrackets);
            } else {
                // 单个器件故障，直接加入到componentSet中
                componentSet.insert(componentsInEntityString);
            }
        }

        // 使用逗号分隔所有器件名称
        functionAllComponent = QStringList(QList<QString>::fromSet(componentSet)).join(",");
...
{
    QString componentDependencyText;
    QStringList allComponentList = allComponent.split(",");
    QStringList linkComponentList = linkText.split(",");
    ...
    if(allComponent!="")
    {
        ...
        {
            QString component = linkComponent.contains(".") ? linkComponent.split(".")[0] : linkComponent;
    ...
    {
        //allComponent为空的情况
        QStringList linkList = linkText.split(",", QString::SkipEmptyParts);
        for (const auto& s : linkList)
        {
            if(s.startsWith("."))continue;
            QString component = s.contains(".") ? s.split(".")[0] : s;
    ...
    QString lastComponent = lastLinkComponent.contains(".") ? lastLinkComponent.split(".")[0] : lastLinkComponent;
    ...
    componentDependencyText = sortedComponentList.join(",");

    return componentDependencyText;
}
