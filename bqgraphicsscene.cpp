...
qreal BQGraphicsScene::SetBackGroundImage(QPixmap pix, int width, int height) {
    this->clear();
    qreal XScale = 1.0;
    qreal YScale = 1.0;
    ...
    if (!pix.isNull()) {
        ...
        XScale = static_cast<qreal>(width) / pix.width(); // 计算宽度缩放比例
        YScale = static_cast<qreal>(height) / pix.height(); // 计算高度缩放比例

        // 设置场景矩形大小为缩放后的图片大小
        this->setSceneRect(0, 0, pix.width(), pix.height());
    }
    m_scaleFactor = qMin(XScale, YScale);
    return m_scaleFactor;
}
bool BQGraphicsScene::SetBackGroundImage(QPixmap pix) {
    this->clear();
    if (!pix.isNull()) {
        QGraphicsPixmapItem* _backGroundItem = this->addPixmap(pix);
